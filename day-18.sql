...
select subject, first_value(score) over(partition by subject order by quiz_date desc) as recent_score,
    first_value(score) over(partition by subject order by quiz_date) as past_score
from daily_quiz_scores group by subject;
