...
WITH annual_snowfall AS ( SELECT resort_id, resort_name, 
  SUM(snowfall_inches) AS annual_snowfall FROM resort_monthly_snowfall
    GROUP BY resort_id, resort_name)
SELECT resort_name, NTILE(4) over (ORDER BY annual_snowfall DESC) as annual_snowfall FROM
annual_snowfall;
